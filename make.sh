...
$CS_COMMAND add-cp.q corpus.txt # Adds CPs to every clause in corpus...yeesh
	# Outputs to: corpus.txt.out 

#2) remove-dup-cp.q on output
$CS_COMMAND remove-dup-cp.q corpus.txt.out # Removes duplicate CPs add in the previous step...yeesh
...
$CS_COMMAND do-support.c external-cp.cod
	# Outputs to: do-support.cod

#6) Run the only-coding.q query (from corpus-tools repository)
$CS_COMMAND only-coding.q do-support.cod
	# Outputs to: do-support.cod.ooo
